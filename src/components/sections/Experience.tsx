...
export default function Experience() {
  return (
    <section id="experience" className="relative w-full py-20">
      {/* full-bleed hero-style background (matches About/Hero) */}
      <div className="absolute inset-0 bg-gradient-to-br from-[#07140f] via-[#0a1b14] to-[#07140f]" />
      <div className="pointer-events-none absolute -left-40 top-24 h-72 w-72 rounded-full bg-emerald-400/10 blur-[100px]" />
      <div className="pointer-events-none absolute -right-24 bottom-0 h-72 w-72 rounded-full bg-emerald-300/10 blur-[100px]" />

      {/* container */}
      <div className="relative z-10 mx-auto grid w-full max-w-7xl gap-6 px-5 sm:px-6 md:px-8">
        {/* heading block — consistent with About */}
        <div>
        ...
        <div className="relative mt-6">
          {/* center spine */}
          <div className="pointer-events-none absolute left-4 top-0 h-full w-px bg-emerald-400/15 md:left-1/2" />
          ...
          <ul className="space-y-12">
            {items.map((it, idx) => {
              ...
              return (
                ...
                >
                  {/* node on the spine */}
                  <span className="absolute left-4 top-8 -translate-x-1/2 md:left-1/2">
                  ...
                  {/* SHORT CAP CURVE — sits above card; no crossing */}
                  <div
                    className={[
                  ...
                  {/* card — same vibe as About (glass on hover) */}
                  <div
                    className={[
                      ...
                      "mt-6 md:mt-10", // gives space for cap curve
                    ].join(" ")}
                  >
                    {/* subtle hover highlight */}
                    <span className="pointer-events-none absolute inset-0 rounded-xl opacity-0 transition-opacity duration-300 group-hover:opacity-100 bg-[radial-gradient(140px_90px_at_10%_0%,rgba(52,211,153,0.15),transparent_70%)]" />
                    ...
                    <div className="mt-1 flex items-center gap-2">
                      ...
                      <svg className="h-4 w-4 flex-none text-emerald-300/80" viewBox="0 0 24 24" fill="none" aria-hidden>
                        <path d="M5 12h12m0 0l-4-4m4 4l-4 4" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
                      </svg>
                    </div>
                    ...
                    {/* hairline separators on hover */}
                    <span className="pointer-events-none absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-white/10 to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
                    <span className="pointer-events-none absolute inset-x-0 bottom-0 h-px bg-gradient-to-r from-transparent via-white/10 to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
                  </div>
                </motion.li>
              );
            })}
